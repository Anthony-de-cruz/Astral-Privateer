...
def edit_map_input():
    ...
    game = AstralPrivateer("Astral Privateer", 1600, 900, COLOURS, 60, map_name)
    ...
    def click(self, clicked: tuple, button: tuple) -> None:
        ...
        if button[0]:
            self.map_data[f"{clicked_x},{clicked_y}"] = [
                "Boundary",
                self.map_data[f"{clicked_x},{clicked_y}"][1],
            ]

        # Right Click
        elif button[2]:
            self.map_data[f"{clicked_x},{clicked_y}"] = [
                f"Buildable_{random.randint(0, 1)}",
                self.map_data[f"{clicked_x},{clicked_y}"][1],
            ]
    ...
    def handle_inputs(self) -> None:
        ...
        for event in self.event_list:
            ...
            if True in pygame.mouse.get_pressed():
                ...
                clicked_sprites = [
                    s for s in self.click_group if s.rect.collidepoint(pos)
                ]
